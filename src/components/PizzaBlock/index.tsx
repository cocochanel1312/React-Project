...
const PizzaBlock: React.FC<IPizzaBlockProps> = ({
  ...
  const onClickTypeName = (typeIndex) => {
    setActiveTypeName(typeIndex);
  };
        ...
        <div className="pizza-block__selector">
          <ul>
            {types.map((typeIndex) => (
              <li
                key={typeIndex}
                onClick={() => onClickTypeName(typeIndex)}
                className={activeTypeName === typeIndex ? "active" : ""}
              >
                {typeNames[typeIndex]}
              </li>
            ))}
